...
class SITTIBilling2 extends StaticPage{
	var $Account;
	var $Paging;
    function SITTIBilling2($req,$account){
    ...
    function save($info){
    	...
    	if(eregi("([0-9]+)",$redeem_code)){
    		...
    		if(!$this->isTopupValueValid($topup)){
    			...
    			$rs = $this->updateMandiriInternet($info,$mandiri_internet,$topup,$token,$kupon);
    			print $rs;
    			/*
	...
	function updateMandiriInternet($info,$mandiri,$topup,$token,$kupon){
    	$this->open(4);
    	$rs = $this->fetch("SELECT sittiID,alt_id FROM db_billing.sitti_account_balance WHERE sittiID='".mysql_escape_string($info['sittiID'])."' LIMIT 1");
    	$this->close();
    	$mandiri = mysql_escape_string($mandiri);
    	...
    	if($rs['sittiID']==$info['sittiID']){
			...
			$out = $api->topupMandiriInternet($info, $mandiri, $topup,$token);
			$js = json_decode($out);
    ...
    function Summary(){
        ...
        if ($req->getParam('campaign')) {
            ...
            $campaignQuery = " AND campaign_id ='".$campaign_id."' ";
    ...
    function topup($profile){  
    	...
    	if($rs['sittiID']==$sittiID){
    		...
    		if(strlen($rs['alt_id'])>0){
    			...
    			return $this->View->toString("SITTI/pembayaran2.html");
    		}else{
