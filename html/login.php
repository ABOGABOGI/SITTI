...
if($direct=="1"){
	...
	if(eregi($CONFIG['WebsiteURL']."konfirmasi.php",$confirm_url)){
	    ...
	    if($smtp->send()){
			$msg = $LOCALE['EMAIL_CONFIRM_RESEND_SUCCESS'];
	    }else{
	    	$msg.= $LOCALE['EMAIL_CONFIRM_RESEND_FAILED'];
	    }
	}else{
		$msg = $LOCALE['EMAIL_CONFIRM_RESEND_EXPIRED'];
	}
	$view->assign("mainContent",$view->showMessage($msg, "http://www.sitti.co.id"));
    print $view->toString("SITTI/content.html");
}else if(!$username || !$password){
    $er = $LOCALE['LOGIN_REQUIRED_FIELD_NOT_ALL_FILLED'];
    ...
    $account->open();
    $check = $account->login($username, $fix_pass, $role);
    ...
    if($check['status']==1){
        ...
        $view->assign("mainContent",$view->showMessageError($msg, "http://www.sitti.co.id"));
        
        print $view->toString("SITTI/content.html");
    }else if($check['status']==3){
    	$_SESSION[urlencode64('block'.date("Ymd"))]+=1;
        $er = $LOCALE['LOGIN_NOT_MATCHED'];
        $sittiHomepage->View->assign("er", $er);
		$sittiHomepage->View->assign("jumpTo", $jumpTo);
