...
if($redirect=="1"){
  	// echo "1: ";print_r ($req->getPost("d"));echo "\n";
	$d = str_replace("\\\\", "", $req->getPost("d"));
	// echo "2: ";print_r (stripslashes($d));echo "\n";
	$_SESSION['adsetskeys'] = base64_encode(stripslashes($d));
	$_SESSION['adbids'] = base64_encode(stripslashes($d));
	// echo "3: ";print_r ($_SESSION['adsetskeys']);echo "\n";
	// echo "4: ";print_r (base64_decode($_SESSION['adsetskeys']));echo "\n";
	// echo "5: ";print_r (json_decode(base64_decode($_SESSION['adsetskeys'])));echo "\n";
	print "1";
	die();
}

if ($req->getParam("get_key")){
	$cat_id = $req->getParam("cat");
	$type = $req->getParam("type");
	$inventory = new SITTIInventory();
	print $inventory->getRelatedKeyword($cat_id,$type);
	die();
}
